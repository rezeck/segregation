	...
	def compute(self, q):
		...
		for i in range(self.ROBOTS):
			for j in range(self.ROBOTS):
				if i == j:
					continue
				if i >= int(math.floor((i) * n_groups)) and i < int(math.floor((i+1) * n_groups)) and j >= int(math.floor((i) * n_groups)) and j < int(math.floor((i+1) * n_groups)):
					dist_same_group.append(self.euclidean(q[i], q[j]))
				else:
					dist_diff_group.append(self.euclidean(q[i], q[j]))
		dist_same_group = np.array(dist_same_group)
		dist_diff_group = np.array(dist_diff_group)
		return [dist_same_group.mean(), dist_diff_group.mean()]
