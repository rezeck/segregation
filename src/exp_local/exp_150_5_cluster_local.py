...
for i in range(len(experiments)):
	...
	plt.errorbar(rads[i], data[:,1].mean(), yerr=data[:,1].std(),  fmt='--o', color='blue', solid_capstyle='projecting', capsize=5)
	plt.errorbar(rads[i], data[:,2].mean(), yerr=data[:,2].std(),  fmt='--o', color='red',solid_capstyle='projecting', capsize=5)
plt.plot(rads, mean_daa, label=r'Average $d_{AA}$', color='blue', linewidth=1)
plt.plot(rads, mean_dab, label=r'Average $d_{AB}$', color='red', linestyle='dashed', linewidth=1)
